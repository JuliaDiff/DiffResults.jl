...
@testset "DiffResult" begin
    ...
    @testset "deprecations" begin
        @testset "value!" begin
            ...
            rn = @test_deprecated DiffResults.value!(rn, n1)
            @test value(rn) === n1
            rn = value!!(rn, n0)

            x0_new, x0_copy = rand(k), copy(x0)
            rx = @test_deprecated DiffResults.value!(rx, x0_new)
            @test value(rx) === x0 == x0_new
            rx = value!!(rx, x0_copy)

            s0_new = rand(k)
            rs = @test_deprecated DiffResults.value!(rs, s0_new)
            @test value(rs) == s0_new
            @test typeof(value(rs)) === typeof(s0)
            rs = value!!(rs, s0)

            rsmix = @test_deprecated DiffResults.value!(rsmix, n1)
            @test value(rsmix) === n1
            rsmix = value!!(rsmix, n0)

            rn = @test_deprecated DiffResults.value!(exp, rn, n1)
            @test value(rn) === exp(n1)
            rn = value!!(rn, n0)

            x0_new, x0_copy = rand(k), copy(x0)
            rx = @test_deprecated DiffResults.value!(exp, rx, x0_new)
            @test value(rx) === x0 == exp.(x0_new)
            rx = value!!(rx, x0_copy)

            s0_new = rand(k)
            rs = @test_deprecated DiffResults.value!(exp, rs, s0_new)
            @test value(rs) == exp.(s0_new)
            @test typeof(value(rs)) === typeof(s0)
            rs = value!!(rs, s0)

            rsmix = @test_deprecated DiffResults.value!(exp, rsmix, n1)
            @test value(rsmix) === exp(n1)
            rsmix = value!!(rsmix, n0)

            ksqrt = Int(sqrt(k))
            T = typeof(SMatrix{ksqrt,ksqrt}(rand(ksqrt,ksqrt)))
            rs_new = @test_deprecated DiffResults.value!(rs, convert(T, value(rs)))
            @test rs_new === rs
        end
        ...
        @testset "derivative!" begin
            ...
            rn = @test_deprecated DiffResults.derivative!(rn, n0)
            @test derivative(rn) === n0
            rn = derivative!!(rn, n1)

            x1_new, x1_copy = rand(k, k), copy(x1)
            rx = @test_deprecated DiffResults.derivative!(rx, x1_new)
            @test derivative(rx) === x1 == x1_new
            rx = derivative!!(rx, x1_copy)

            s1_new = rand(k, k)
            rs = @test_deprecated DiffResults.derivative!(rs, s1_new)
            @test derivative(rs) == s1_new
            @test typeof(derivative(rs)) === typeof(s1)
            rs = derivative!!(rs, s1)

            s0_new = rand(k)
            rsmix = @test_deprecated DiffResults.derivative!(rsmix, s0_new)
            @test derivative(rsmix) == s0_new
            @test typeof(derivative(rsmix)) === typeof(s0)
            rsmix = derivative!!(rsmix, s0)

            rn = @test_deprecated DiffResults.derivative!(rn, n1, Val{2})
            @test derivative(rn, Val{2}) === n1
            rn = derivative!!(rn, n2, Val{2})

            x2_new, x2_copy = rand(k, k, k), copy(x2)
            rx = @test_deprecated DiffResults.derivative!(rx, x2_new, Val{2})
            @test derivative(rx, Val{2}) === x2 == x2_new
            rx = derivative!!(rx, x2_copy, Val{2})

            s2_new = rand(k, k, k)
            rs = @test_deprecated DiffResults.derivative!(rs, s2_new, Val{2})
            @test derivative(rs, Val{2}) == s2_new
            @test typeof(derivative(rs, Val{2})) === typeof(s2)
            rs = derivative!!(rs, s2, Val{2})

            s1_new = rand(k, k)
            rsmix = @test_deprecated DiffResults.derivative!(rsmix, s1_new, Val{2})
            @test derivative(rsmix, Val{2}) == s1_new
            @test typeof(derivative(rsmix, Val{2})) === typeof(s1)
            rsmix = derivative!!(rsmix, s1, Val{2})

            rn = @test_deprecated DiffResults.derivative!(exp, rn, n0)
            @test derivative(rn) === exp(n0)
            rn = derivative!!(rn, n1)

            x1_new, x1_copy = rand(k, k), copy(x1)
            rx = @test_deprecated DiffResults.derivative!(exp, rx, x1_new)
            @test derivative(rx) === x1 == exp.(x1_new)
            rx = derivative!!(exp, rx, x1_copy)

            s1_new = rand(k, k)
            rs = @test_deprecated DiffResults.derivative!(exp, rs, s1_new)
            @test derivative(rs) == exp.(s1_new)
            @test typeof(derivative(rs)) === typeof(s1)
            rs = derivative!!(exp, rs, s1)

            s0_new = rand(k)
            rsmix = @test_deprecated DiffResults.derivative!(exp, rsmix, s0_new)
            @test derivative(rsmix) == exp.(s0_new)
            @test typeof(derivative(rsmix)) === typeof(s0)
            rsmix = derivative!!(exp, rsmix, s0)

            rn = @test_deprecated DiffResults.derivative!(exp, rn, n1, Val{2})
            @test derivative(rn, Val{2}) === exp(n1)
            rn = derivative!!(rn, n2, Val{2})

            x2_new, x2_copy = rand(k, k, k), copy(x2)
            rx = @test_deprecated DiffResults.derivative!(exp, rx, x2_new, Val{2})
            @test derivative(rx, Val{2}) === x2 == exp.(x2_new)
            rx = derivative!!(exp, rx, x2_copy, Val{2})

            s2_new = rand(k, k, k)
            rs = @test_deprecated DiffResults.derivative!(exp, rs, s2_new, Val{2})
            @test derivative(rs, Val{2}) == exp.(s2_new)
            @test typeof(derivative(rs, Val{2})) === typeof(s2)
            rs = derivative!!(exp, rs, s2, Val{2})

            s1_new = rand(k, k)
            rsmix = @test_deprecated DiffResults.derivative!(exp, rsmix, s1_new, Val{2})
            @test derivative(rsmix, Val{2}) == exp.(s1_new)
            @test typeof(derivative(rsmix, Val{2})) === typeof(s1)
        ...
        @testset "gradient!" begin
            x1_new, x1_copy = rand(k, k), copy(x1)
            rx = @test_deprecated DiffResults.gradient!(rx, x1_new)
            @test gradient(rx) === x1 == x1_new
            rx = gradient!!(rx, x1_copy)

            s1_new = rand(k, k)
            rs = @test_deprecated DiffResults.gradient!(rs, s1_new)
            @test gradient(rs) == s1_new
            @test typeof(gradient(rs)) === typeof(s1)
            rs = gradient!!(rs, s1)

            x1_new, x1_copy = rand(k, k), copy(x1)
            rx = @test_deprecated DiffResults.gradient!(exp, rx, x1_new)
            @test gradient(rx) === x1 == exp.(x1_new)
            rx = gradient!!(exp, rx, x1_copy)

            s0_new = rand(k)
            rsmix = @test_deprecated DiffResults.gradient!(exp, rsmix, s0_new)
            @test gradient(rsmix) == exp.(s0_new)
            @test typeof(gradient(rsmix)) === typeof(s0)
            rsmix = gradient!!(rsmix, s0)

            T = typeof(SVector{k*k}(rand(k*k)))
            rs_new = @test_deprecated DiffResults.gradient!(rs, convert(T, gradient(rs)))
            @test rs_new === rs
        end

        @testset "jacobian!"  begin
            x1_new, x1_copy = rand(k, k), copy(x1)
            rx = @test_deprecated DiffResults.jacobian!(rx, x1_new)
            @test jacobian(rx) === x1 == x1_new
            rx = jacobian!!(rx, x1_copy)

            s1_new = rand(k, k)
            rs = @test_deprecated DiffResults.jacobian!(rs, s1_new)
            @test jacobian(rs) == s1_new
            @test typeof(jacobian(rs)) === typeof(s1)
            rs = jacobian!!(rs, s1)

            x1_new, x1_copy = rand(k, k), copy(x1)
            rx = @test_deprecated DiffResults.jacobian!(exp, rx, x1_new)
            @test jacobian(rx) === x1 == exp.(x1_new)
            rx = jacobian!!(exp, rx, x1_copy)

            s0_new = rand(k)
            rsmix = @test_deprecated DiffResults.jacobian!(exp, rsmix, s0_new)
            @test jacobian(rsmix) == exp.(s0_new)
            @test typeof(jacobian(rsmix)) === typeof(s0)
            rsmix = jacobian!!(rsmix, s0)

            T = typeof(SVector{k*k}(rand(k*k)))
            rs_new = @test_deprecated DiffResults.jacobian!(rs, convert(T, jacobian(rs)))
            @test rs_new === rs
        end

        @testset "hessian!" begin
            x2_new, x2_copy = rand(k, k, k), copy(x2)
            rx = @test_deprecated DiffResults.hessian!(rx, x2_new)
            @test hessian(rx) === x2 == x2_new
            rx = hessian!!(rx, x2_copy)

            s2_new = rand(k, k, k)
            rs = @test_deprecated DiffResults.hessian!(rs, s2_new)
            @test hessian(rs) == s2_new
            @test typeof(hessian(rs)) === typeof(s2)
            rs = hessian!!(rs, s2)
            ...
            s1_new = rand(k, k)
            rsmix = @test_deprecated DiffResults.hessian!(exp, rsmix, s1_new)
            @test hessian(rsmix) == exp.(s1_new)
            @test typeof(hessian(rsmix)) === typeof(s1)
            rsmix = hessian!!(rsmix, s1)

            T = typeof(SVector{k*k*k}(rand(k*k*k)))
            rs_new = @test_deprecated DiffResults.hessian!(rs, convert(T, hessian(rs)))
            @test rs_new === rs
